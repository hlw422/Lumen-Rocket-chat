...
export class ApiClient {
  ...
  request<R = any, T = any>(url: string, method?: string): ApiFunction<R, T> {
    return async (params: R, options?: ApiOptions): Promise<T> => {
      const mergedMethod = options?.method || method || this.defaultOptions.method
      const isGet = mergedMethod === 'GET'

      // GET 请求时将 params 序列化为 query string
      let finalUrl = this.baseUrl + url
      if (isGet && params && typeof params === 'object' && !(params instanceof FormData)) {
      ...
      for (const interceptor of this.interceptor.request.interceptors) {
        req = await interceptor(finalUrl, req)
      }
      ...
      const to = async () => {
        ...
        try {
          ...
          const resp = await fetch(finalUrl, req)
